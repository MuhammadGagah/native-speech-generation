...
# Ensure we can import our local modules (lib_updater)
...
if not os.path.isdir(lib_dir):
    ...
    # Note: We might need to ensure lib_dir is inserted before other paths if needed,
    # but sys.path.insert(0, ...) works.
...
if not LIBS_AVAILABLE:
    ...
    class GlobalPlugin(globalPluginHandler.GlobalPlugin):
        ...
        def script_openDialog(self, gesture) -> None:
            # Translators: Shown when the add-on is not ready yet and requires NVDA restart.
            wx.CallAfter(
                wx.MessageBox,
                _("Native Speech Generation is installing dependencies. "
    ...
    try:
        ...
        # 2. Force unload to allow import of our local version
        if 'typing_extensions' in sys.modules:
            del sys.modules['typing_extensions']
        
        # 3. Import dependencies that need the NEW version
        # This will load typing_extensions from our lib because sys.path[0] is our lib
        try:
            from google import genai
            from google.genai import types
            GENAI_AVAILABLE = True
            log.info("google-genai loaded successfully with local typing_extensions.")
        finally:
             # 4. Restore the original NVDA module to sys.modules
             # so other parts of NVDA assume nothing changed.
             if original_typing_ext:
                 sys.modules['typing_extensions'] = original_typing_ext
             # Note: 'genai' module will still hold a reference to the *new* typing_extensions class
             # internally, which is exactly what we want (Local Isolation).
    ...
    class NativeSpeechSettingsPanel(SettingsPanel):
        ...
        def onReinstall(self, evt: wx.Event) -> None:
            ...
            try:
                # Use the centralized LIB_DIR from lib_updater if available, else derive it
                try:
                    import lib_updater
                    target_lib = lib_updater.LIB_DIR
                except ImportError:
                    # Fallback
                    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                    target_lib = os.path.join(base, "globalPlugins", "lib")

                if os.path.exists(target_lib):
                    import shutil
                    import time
                    # Rename first to avoid lock issues, then delete
                    temp_trash = target_lib + "_trash_" + str(time.time())
                    os.rename(target_lib, temp_trash)
                    # We don't need to try removing immediately if it fails, startup cleanup catches it.
                    # But we can try.
        ...
        def onToggleApiVisibility(self, event) -> None:
    ...
    class NativeSpeechDialog(wx.Dialog):
        ...
        def _generate_thread(self, text: str) -> None:
            ...
            try:
                asyncio.run(self._generate_async(text))
            except Exception as e:
                ui.message(_("An error occurred during generation."))
                log.error(f"Unexpected error in generate_thread: {e}", exc_info=True)
                wx.CallAfter(wx.MessageBox, f"An unexpected error occurred: {e}", _("Error"), wx.OK | wx.ICON_ERROR)
            finally:
                wx.CallAfter(self._restore_generate_button)

        async def _generate_async(self, text: str) -> None:
            try:
                # Use aiohttp for faster async requests (automatically handled by SDK if installed)
                client = genai.Client(api_key=self.api_key)
            except Exception as e:
                log.error(f"Failed init genai client: {e}", exc_info=True)
                wx.CallAfter(wx.MessageBox, f"Failed to initialize Google GenAI client: {e}", _("Error"), wx.OK | wx.ICON_ERROR)
                return
            ...
            try:
                ...
                saved_path = await self._stream_and_save_audio_async(client, self.model, contents, generate_config, out_path_base)
        ...
        async def _stream_and_save_audio_async(
            self,
            client: Any, 
            model: str, 
            contents: Any, 
            ...
            try:
                # Streaming with Async Client
                stream = await client.aio.models.generate_content_stream(model=model, contents=contents, config=config_obj)
                async for chunk in stream:
                    ...
                    if part.inline_data and getattr(part.inline_data, 'data', None):
                        ...
                        if not ext or ext.lower() not in (".wav", ".mp3", ".ogg", ".flac"):
                            # This convert_to_wav helper is synchronous/CPU bound, ok to run here
                ...
                wx.CallAfter(wx.MessageBox, f"Failed to generate speech: {e}", _("Error"), wx.OK | wx.ICON_ERROR)
                return None
            ...
            with wx.FileDialog(
                ...
                try:
                    ...
                    with open(self.last_audio_path, "rb") as src, open(dest, "wb") as dst:
                        dst.write(src.read())
                    wx.CallAfter(wx.MessageBox, f"Audio saved to {dest}", _("Success"), wx.OK | wx.ICON_INFORMATION)
                except Exception as e:
                    wx.CallAfter(wx.MessageBox, f"Failed to save audio: {e}", _("Error"), wx.OK | wx.ICON_ERROR)
    ...
    class GlobalPlugin(globalPluginHandler.GlobalPlugin):
        """
        The main Global Plugin class for Native Speech Generation.
        """
        def __init__(self):
            super().__init__()
            ...
            if IMPORT_ERROR_MSG:
                wx.CallAfter(
                    wx.MessageBox,
                    _("Native Speech Generation detected an installation issue:\n\n") + IMPORT_ERROR_MSG,
                    _("Dependency Error"),
                    wx.OK | wx.ICON_ERROR
                )
        ...
        def onShowDialog(self, evt) -> None:
        ...
        def _openDialog(self) -> None:
            ...
            try:
                ...
                wx.CallAfter(
                    wx.MessageBox,
                    f"Failed to open Native Speech Generation dialog: {e}",
                    _("Error"),
                    wx.OK | wx.ICON_ERROR
                )

        def onDialogClose(self, event) -> None:
